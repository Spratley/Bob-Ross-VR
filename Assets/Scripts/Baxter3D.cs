...
public class Baxter3D : MonoBehaviour
{
	...
	void UpdateShaderBAXTER(){
		...
		if (pointsOnCanvas.Count > 0) {
			//Put contact position into local canvas space
			var pos = ExtraMath.AveragePosition(pointsOnCanvas) - transform.position;
			...
			Vector2 textureCoord = (Vector2)pos + new Vector2(0.5f, 0.5f);
			textureCoord.x = 1 - textureCoord.x;
			textureCoord.y = (1 - textureCoord.y);

				float IDX,IDY;
				mousePosition 	  = textureCoord;
				IDX = textureCoord.x - mousePositionPrev.x;
				IDY = textureCoord.y - mousePositionPrev.y;
			...
			//angle = m_brushAngle;
				...
				shader.SetFloat("InputX", textureCoord.x);
				shader.SetFloat("InputY", textureCoord.y);
				...
				//if(MOUSE_DOWN || MOUSE_DOWN_PIGMENT)
				{
					for(int i=-bw;i<bw;i++)
					{
						for(int j=-bw;j<bw;j++)
						{
							...
							float bx = (float)(textureCoord.x*width);
							float by = (float) (textureCoord.y*height);
							float xR = (float)i;
							float yR = (float)j;
